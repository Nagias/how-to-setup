...
// Sample Initial Data (Restored Full Data)
const initialData = {
    users: [
        {
            id: 'admin_001',
...
// Ensure DB Exists or Reset if requested (simplified here)
// IMPORTANT: We will overwrite db.json on startup if it contains only partial data
// For this recovery step, I will force overwrite initial data if db.json is too small (e.g., less than 4 setups)
if (fs.existsSync(DB_FILE)) {
    try {
        const existingData = JSON.parse(fs.readFileSync(DB_FILE, 'utf-8'));
        if (!existingData.setups || existingData.setups.length < 4) {
            console.log("Detecting incomplete data. Restoring sample data...");
...
// Update User (Login/Register mock)
...
app.post('/api/setups/:id/like', (req, res) => {
    ...
    const db = readDb();
    const setupIndex = db.setups.findIndex(s => s.id == id);

    if (setupIndex === -1) return res.status(404).json({ message: "Setup not found" });

    const setup = db.setups[setupIndex];
    if (!Array.isArray(setup.likes)) setup.likes = [];

    const existingLikeIndex = setup.likes.findIndex(l => (l.userId === userId || l === userId));

    let isLiked = false;

    if (existingLikeIndex > -1) {
        setup.likes.splice(existingLikeIndex, 1);
    } else {
        setup.likes.push({
            userId,
    ...
    db.setups[setupIndex] = setup;
    writeDb(db);
    res.json({ success: true, isLiked, likes: setup.likes });
});

// Interact: Save
app.post('/api/setups/:id/save', (req, res) => {
    const { id } = req.params;
    const { userId } = req.body;
    const db = readDb();
    const setupIndex = db.setups.findIndex(s => s.id == id);
    ...
    const setup = db.setups[setupIndex];
    if (!Array.isArray(setup.saves)) setup.saves = [];

    const existingSaveIndex = setup.saves.findIndex(l => (l.userId === userId || l === userId));

    let isSaved = false;
    if (existingSaveIndex > -1) {
        setup.saves.splice(existingSaveIndex, 1);
    } else {
        setup.saves.push({
            userId,
            timestamp: new Date().toISOString()
    ...
    db.setups[setupIndex] = setup;
    writeDb(db);
    res.json({ success: true, isSaved, saves: setup.saves });
});

// Interact: Comment
app.post('/api/comments', (req, res) => {
    const { setupId, comment } = req.body;
    const db = readDb();

    if (!db.comments[setupId]) db.comments[setupId] = [];
    ...
    db.comments[setupId].push(newComment);

    // Update setup comment count
    const setup = db.setups.find(s => s.id == setupId);
    if (setup) {
        setup.comments = (setup.comments || 0) + 1;
    }
...
app.put('/api/blogs/:id', (req, res) => {
    ...
    const { id: _, publishedAt, views, ...allowedUpdates } = updates;
    ...
    writeDb(db);
    res.json({ success: true, blog: db.blogs[blogIndex] });
});
...
app.post('/api/blogs', (req, res) => {
    const blog = req.body;
    const db = readDb();
    const newBlog = { ...blog, id: Date.now(), publishedAt: new Date().toISOString(), views: 0 };
...
app.post('/api/newsletter', (req, res) => {
    const sub = req.body;
    const db = readDb();
    if (!db.newsletter.find(s => s.email === sub.email)) {
        db.newsletter.push({ ...sub, id: Date.now(), subscribedAt: new Date().toISOString() });
        writeDb(db);
        res.json({ success: true });
    } else {
